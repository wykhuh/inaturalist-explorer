...
class MyComponent extends HTMLElement {
  ...
  async render() {
    ...
    await setupComponent(
      "/src/components/SelectedUsersIdentifiersItem/template.html",
      this,
    );
    ...
    if (butttonEl) {
      butttonEl.addEventListener("click", async function () {
        if (user.id !== undefined) {
          await removeUserIdentifier(window.app.store);
        }
      });
    }
  }
...
customElements.define("x-users-identifiers-list-item", MyComponent);
